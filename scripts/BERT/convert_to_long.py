...
import datasets
from transformers import TrainingArguments, HfArgumentParser, LongformerSelfAttention
import logging
import os
import math
import copy
...
class RobertaLongForMaskedLM(RobertaForMaskedLM):
    def __init__(self, config):
        ...
        self.roberta.encoder.layer = nn.ModuleList(
            [
                RobertaLongSelfAttention(config, layer_id=index)
...
def create_long_model(
    ...
    tokenizer = RobertaTokenizerFast.from_pretrained(
        initialization_tokenizer, model_max_length=max_pos
    )
...
model_path = f"{training_args.output_dir}/roberta-base-ecthr-{model_args.max_pos}"

model, tokenizer = create_long_model(
    initialization_model=roberta_base_path,
    initialization_tokenizer=roberta_base_tokenizer_path,
...
pretrain_and_evaluate(
    training_args, model, tokenizer, eval_only=False, model_path=model_path
)
